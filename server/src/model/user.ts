...
export class UserRepository extends Loggable {
    public static async userAdd(user: IUser) {
        ...
        const doc = {username, name, password, password2, hashpassword};
        const query = "select * from UserAddJSON($1) res";
        const queryRes = await DB.db().one<IQueryResult>(query, [doc]);
    ...
    public static async userUpdate(user: IUser) {
        ...
        const doc = { username,  name,  password, password2, hashpassword, id };
        BowLog.log2(myself, "doc=");
        BowLog.dir(myself, doc);

        const query = "select * from UserUpdateJSON($1) res";
        const queryRes = await DB.db().one<IQueryResult>(query, [doc]);
    ...
    public static async userDelete(id: string) {
        const myself = this.getMyself("userDelete");
        BowLog.log1(myself, " id=" + id);

        const query = "select * from userDeleteJSON($1) res";
        const queryRes = await DB.db().one<IQueryResult>(query, [id]);
    ...
    public static async userGet(id: string) {
        const myself = this.getMyself("userGet");
        BowLog.log1(myself, " id=" + id);

        const query = "select * from userGetJSON($1) res";
        const queryRes = await DB.db().one<IQueryResultWithPayload<IUser>>(query, [id]);
    ...
    public static async userList() {
        ...
        const query = "select * from userListJSON() res";
        const queryRes = await DB.db().one<IQueryResultWithPayload<IUserInfo[]>>(query);
    ...
    public static async userLogin(login: ILogin) {
        ...
        const hashpassword = await UserRepository.hashPassword(password);
        const doc = {username, hashpassword};
        BowLog.log2(myself, "doc=");
        BowLog.dir(myself, doc);

        const query = "select * from UserLoginJSON($1) res";
        const queryRes = await DB.db().one<IQueryResultWithPayload<IUserInfo>>(query, [doc]);
