...
create or replace function StandardDeleteActionJSON(entity varchar(40))
returns JSONB
as $$
declare
_js JSONB;
...
select * from jsetstr(_js, 'query', concat(entity,'DeleteJSON($1)') ) into _js;
select * from jsetstr(_js, 'confirm', concat('Are you sure you want to delete ', entity, ' ${name}?' ) ) into _js;
